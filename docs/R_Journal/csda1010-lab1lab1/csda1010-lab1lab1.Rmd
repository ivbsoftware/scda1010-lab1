...
Abstract: >
The specific problem under consideration is to rank selection of applicants for nursery schools in Ljubljana, Slovenia in the 1980's. 
Nursery Database was derived from a hierarchical decision model originally developed to rank applications.
A classification model has been used to developed a reliable recomendation algorithm to predict if a specific applicant is a suitable candidate to be addmitted into a nursery school. 
...
During this time "new housing developments populated by young families, the demand for children’s admission in nursery schools outstrips supply,    notwithstanding the fast growth rate of new schools."
...
The objective is to provide a reliable recommendation algorithm to predict if a specific applicant is a suitable candidate to be admitted into a nursery school or if the applicant should stay with their parents. The results of this recommendation may affect the child’s engagement within the school, parents’ involvement in school activities and overall satisfaction of the applicants long-term academic progress.
...
The following tables presents the input attributes description with their structure:
...
## Plan
To solve the objective a group of four students calling themselves The First Group (T.F.G) from York University School of Continuing Studies, have come together to create an algorithm to test and train a classification model using the Nursery dataset. 

The main tool used in developing the recomendation algorithm is R. The R language is widely used among statisticians and data miners for developing statistical software and data analysis.


# Data understanding
Being able to have a positive outcome from the classification model, preparation of the Nursery dataset followed by descriptive satistic will be the first key step in producing a Random Forest model.
...
We can conclude that there is no missing information and dataset is in good standing to be used.
...
```
Distribution of Class attribute presented in Figure2 \ref{fig:classd} shows the targeted variables with five segments. The frequency of the segment "Recommend" is low (0.00015%) and will not be taken into consideration in the final dataset.
...
The following code (Figure \ref{fig:classdp}) renders distribution of class attribute depending on parents attribute. From this graph, it clearly shows that priority and recommendation are given to application from parent having usual and pretentious occupations. 
...
The following code (Figure \ref{fig:classdh}) renders distribution of class attribute depending on 'health' attribute. Being healthy is most important and relevant for parents to have their nursery application being recommended. Parents having strong and good social situation are more likely to have their application being recommended.
...
ggplot(data = nursery_data, mapping = aes(x = class, fill = health)) +
  geom_bar()
```
The following code (Figure \ref{fig:classdi}) renders distribution of class attribute depending on 'social' attribute. 
...
```
The following code (Figure \ref{fig:classdi}) renders distribution of 'class' attribute depending on 'nursery' attribute. Parents in segments "Proper" and "less_Proper" appear to have similar distribution and their application taken as priority.
...
The Nursery dataset has been split in such a way that train and test sets would have the same distribution of the 'class' attribute. The reason for this is to focus on the priority of an applicants placement in a nursery school rather than an applicants family or social status.
...
The following code (Figure \ref{fig:testd}) renders distribution of 'class' attribute depending on 'nursery' attribute. ...
...
As a first step we have train a Decision Tree model. Though this model is known to be computationally fast but not very precise, we have use all default parameters and all attributes of the train dataset in the resulting Decision Tree presented in Figure \ref{fig:dtree}. This tree shows that..  
...
Importance of the dataset attributes for the prediction of the 'class' attribute shown in Figure \ref{fig:forimp}. 
...
# Conclusion
Through exploring the Nursery Dataset and using a classification model to developed our algorithm to predict applicants suitabilty of an admittance within the nursery school system, the evaluation of the model results indicated the following:
